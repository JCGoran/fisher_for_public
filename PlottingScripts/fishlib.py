...
class GitEnv(object):
    ...
    def get_commit(self):
        cmd = sp.Popen(self.get_git_cmd(['log', '-n','1']), stdout=sp.PIPE, encoding='utf8')
    ...
    def get_remote(self):
        cmd = sp.Popen(self.get_git_cmd(['remote', '-v']), stdout=sp.PIPE, encoding='utf8')
        cmd_out, cmd_err = cmd.communicate()
        if bool(cmd_out):
            try:
    ...
    def get_branch(self):
        cmd = sp.Popen(self.get_git_cmd(['branch']), stdout=sp.PIPE, encoding='utf8')
        cmd_out, cmd_err = cmd.communicate()
    ...
    def get_repo(self):
        cmd = sp.Popen(self.get_git_cmd(['rev-parse','--show-toplevel']), stdout=sp.PIPE, encoding='utf8')
        cmd_out, cmd_err = cmd.communicate()
